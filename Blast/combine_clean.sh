...
cd  $RESULTS
cat blast_"$1"_hits_*|grep query|head -n 1 > combine_best_hits.txt
cat blast_"$1"_hits_*|grep -v query|awk ' $12 > 500' >> combine_best_hits.txt
cat blast_"$1"_hits_*|grep -v query|awk ' $12 > 500 {print $1}'|sort -u > Protein_best_hits.txt
