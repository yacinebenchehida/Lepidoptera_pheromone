...
combine_and_align_genes() {
    ...
    for sp in $SPECIES; do
        python3 ./unique_sequence.py "$INPUTS/${sp}/${PHE}/${sp}_${PHE}_proteins.fasta" "$INPUTS/${sp}/${PHE}/${sp}_${PHE}_unique_cds.fasta"
        cat "$INPUTS/${sp}/${PHE}/${sp}_${PHE}_unique_cds.fasta" | awk -v species="_${sp}" '/^>/ {split($1, a, " "); print a[1] species; next} {print}' >> "$combined_file"
    done
